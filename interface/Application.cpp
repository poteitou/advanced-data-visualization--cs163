    ...
    : mWindow(sf::VideoMode(1600, 900), "Data Structure Visualization", sf::Style::Titlebar | sf::Style::Close)
{
    ...
    mFontCode.loadFromFile("resources/fonts/code.ttf");
...
{
    sf::Clock clock;
    Screen mScreen(mWindow, mFont, mFontCode);
    ...
    while (mWindow.pollEvent(event))
    {
        ...
        case sf::Event::TextEntered:
            if (event.text.unicode == 8) // Backspace
                keyPress = '@';
            else if ((event.text.unicode >= 48 && event.text.unicode <= 57) || (event.text.unicode >= 97 && event.text.unicode <= 122) || (event.text.unicode >= 65 && event.text.unicode <= 90) || event.text.unicode == 32 || event.text.unicode == 46) // 0 -> 9 or space, dot
